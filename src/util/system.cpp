...
const char * const BITCOIN_CONF_FILENAME = "bilcoin.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "bilcoind -foo=bar
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bilcoin-tx using stdin
        std::string val;
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Bilcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Bilcoin
    // Mac: ~/Library/Application Support/Bilcoin
    // Unix: ~/.bilcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bilcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Bilcoin";
#else
    // Unix
    return pathRet / ".bilcoin";
#endif
#endif
}
