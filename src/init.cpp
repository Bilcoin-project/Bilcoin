 ...
 */
static const char* BITCOIN_PID_FILENAME = "bilcoind.pid";
...
{
    const std::string URL_EXPLORER= "<https://explorer.bilcoin.com>";
    const std::string URL_WEBSITE = "<https://bilcoin.com>";
    const std::string URL_MININGPOOL = "</>";
    const std::string URL_WEBWALLET = "</>";
    const std::string URL_TELEGRAM = "</>";
    const std::string URL_GITHUB = "</>";
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (fs::exists(config_file_path)) {
        ...
        if (configFile != NULL) {
            std::string strHeader = "# Bilcoin config file:\n"
                                    "rpcuser=username\n"
                                    ...
                                    "\n"
                                    "# Addnodes:\n"
                                    "addnode=seed01.bilcoin.com\n"
                                    "addnode=seed02.bilcoin.com\n"
                                    "addnode=seed03.bilcoin.com\n"
                                    "addnode=seed04.bilcoin.com\n"
                                    "\n";
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if bilcoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if bilcoin is started while in a temporary directory.\n",
