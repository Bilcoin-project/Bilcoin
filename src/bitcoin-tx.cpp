...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " bilcoin-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  bilcoin-tx [options] <hex-tx> [commands]  Update hex-encoded bilcoin transaction\n" +
            "or:     bilcoin-tx [options] -create [commands]   Create hex-encoded bilcoin transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded bilcoin transaction
