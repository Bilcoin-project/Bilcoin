...
1. Clone the Bilcoin source code:
    ```shell
    git clone https://github.com/Bilcoin/Bilcoin
    cd bilcoin
    ```

2.  Build Bilcoin:

    Configure and build the headless Bilcoin binaries as well as the GUI (if Qt is found).
...
## `disable-wallet` mode
When the intention is to run only a P2P node without a wallet, Bilcoin may be
...
## Running
Bilcoin is now available at `./src/bilcoind`

Before running, you may create an empty configuration file:
```shell
mkdir -p "/Users/${USER}/Library/Application Support/Bilcoin"

touch "/Users/${USER}/Library/Application Support/Bilcoin/bilcoin.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Bilcoin/bilcoin.conf"
```

The first time you run bilcoind, it will start downloading the blockchain. This process could
take many hours, or even days on slower than average systems.

You can monitor the download process by looking at the debug.log file:
```shell
tail -f $HOME/Library/Application\ Support/Bilcoin/debug.log
```

## Other commands:
```shell
./src/bilcoind -daemon      # Starts the bilcoin daemon.
./src/bilcoin-cli --help    # Outputs a list of command-line options.
./src/bilcoin-cli help      # Outputs a list of RPC commands when the daemon is running.
```
...
- The Apple keyholder uses this unsigned app to create a detached signature, using the
  script that is also included there. Detached signatures are available from this [repository](https://github.com/Bilcoin/Bilcoin-detached-sigs).
